...
router.post('/', (req, res) => {
    ...
    const companyJobRef = (d.company_job_reference && d.company_job_reference.trim() !== '') ? d.company_job_reference.trim() : '';

    generateProjectNumber(d.type || 'PV', (aresProjNum) => {
        const sql = `INSERT INTO installations (
            ...
            created_date, status, cert_status, project_number, company_job_reference
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
        db.run(sql, [
            ...
            getSydneyISO(), 'Pending', 'Pending', aresProjNum, companyJobRef
        ], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            addHistory(this.lastID, 'Created', `Installation added with Reference #${aresProjNum} for ${d.first_name} ${d.last_name}.`, currentUser);
            res.json({ id: this.lastID, project_number: aresProjNum, company_job_reference: companyJobRef, success: true });
        });
    });
});
...
router.put('/:id', (req, res) => {
    const d = req.body;
    const currentUser = d.currentUser || 'System';
    const companyJobRef = (d.company_job_reference !== undefined) ? d.company_job_reference.trim() : '';

    db.get('SELECT project_number, type FROM installations WHERE id = ?', [req.params.id], (err, existing) => {
        if (err || !existing) return res.status(500).json({ error: 'Installation not found' });

        const performUpdate = (finalProjNum) => {
            const sql = `UPDATE installations SET
                type=?, company=?, first_name=?, last_name=?, phone=?, email=?,
                google_address=?, unit_number=?, lot_number=?, street_type=?,
                address=?, suburb=?, state=?, postcode=?, project_number=?, company_job_reference=?
                WHERE id=?`;
            db.run(sql, [
                d.type || '', d.company || '', d.first_name || '', d.last_name || '',
                d.phone || '', d.email || '', d.google_address || '',
                d.unit_number || '', d.lot_number || '', d.street_type || '',
                d.address || '', d.suburb || '', d.state || '', d.postcode || '',
                finalProjNum, companyJobRef,
                req.params.id
            ], (updateErr) => {
                if (updateErr) return res.status(500).json({ error: updateErr.message });
                addHistory(req.params.id, 'Edited', 'Installation details updated.', currentUser);
                res.json({ success: true, project_number: finalProjNum, company_job_reference: companyJobRef });
            });
        };

        if (existing.project_number && existing.project_number !== 'Pending') {
            performUpdate(existing.project_number);
        } else {
            generateProjectNumber(d.type || existing.type || 'PV', (newAresNum) => {
                performUpdate(newAresNum);
            });
        }
    });
});
