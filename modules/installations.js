...
router.post('/', (req, res) => {
    ...
    const executeInsert = (finalProjNum) => {
        ...
        db.run(sql, [
            ...
            getSydneyISO(), 'Pending', 'Pending', finalProjNum
        ], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            addHistory(this.lastID, 'Created', `Installation added with Reference #${finalProjNum} for ${d.first_name} ${d.last_name}.`, currentUser);
            res.json({ id: this.lastID, project_number: finalProjNum, success: true });
        });
    };

    if (projNum) {
        executeInsert(projNum);
    } else {
        generateProjectNumber(d.type || 'PV', (autoGeneratedNum) => {
            executeInsert(autoGeneratedNum);
        });
    }
});
